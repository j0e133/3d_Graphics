...
# load cube
mesh = Mesh()
mesh.load('3dObjects/cube.mesh')

# add the sphere to the scene
faces = []
for x in range(-2, 3):
    for y in range(-2, 3):
        faces.extend(Object(mesh, vector3(x * 3, 0, y * 3), 3 / 2, lightPos))
...
while True:
    ...
    camera.move(vector3.sum(funcs[key]() * SPEED * dt for key in pressed if pressed[key]))
